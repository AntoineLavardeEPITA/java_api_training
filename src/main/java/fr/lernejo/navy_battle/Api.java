...
public class Api extends Client {
    ...
    public void start(int port, String url) throws IOException {
        ...
        server.createContext("/api/game/start", s -> handleStartGame(new HttpHandler(s)));
        server.createContext("/api/game/fire", s -> handleFire(new HttpHandler(s)));
        server.start();
        if (url != null)
    ...
    public void handleStartGame(HttpHandler handler) throws IOException {
        try {
    ...
    public void handleFire(HttpHandler handler) throws IOException {
        try {
